...
export async function reserveTurn(args: {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.execute(sql`
      ...
      DO UPDATE SET
        tokens_in = agent.rate_limits.tokens_in + EXCLUDED.tokens_in,
        turns = agent.rate_limits.turns + EXCLUDED.turns
    `);
  });
...
export async function commitActualTokens(args: {
  ...
  if (rows.length === 0) {
    console.warn('[agent.rate-limit.commit.missing-row]', {
      tenantId: args.tenantId,
      userId: args.userId,
