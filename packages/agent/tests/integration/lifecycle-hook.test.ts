...
const baseRunStarted = (overrides: Partial<MastraLifecycleEvent> = {}): MastraLifecycleEvent =>
  ({
    ...
    runId: FIXED_RUN_ID,
    eventSeq: 1,
    workflowId: 'agent.test-workflow',
...
describe('onLifecycleEvent — outbox emission', () => {
  it('writes a core.events row for each outbox-eligible kind', async () => {
    await withAgentTestDb(async ({ pool }) => {
      ...
      expect(evts.rows.map((r) => r.event_type)).toEqual([
        'agent.workflow.approval.requested',
        'agent.workflow.run.completed',
      ]);
  ...
  it('writes agent.workflow.run.failed for a run-failed terminal event', async () => {
    await withAgentTestDb(async ({ pool }) => {
      const runId = randomUUID();
      const tenantId = randomUUID();
      const userId = randomUUID();
      ...
      expect(evts.rows.map((r) => r.event_type)).toEqual(['agent.workflow.run.failed']);
